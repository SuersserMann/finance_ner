...
def collate_fn(data):
    ...
    for i in data:
        ...
        event_one = i[2]
        # events.append(i[2])
        # 这里面去掉空格的原因是因为token的时候空格会被自动忽略而不被计入，如果想计算空格应该用特殊字符代替空格，之后再用空格替换该特殊字符
    ...
    try:
        for epoch in range(num_epochs):
            ...
            if val_f1 > best_val_f1:
                ...
                torch.save(model.state_dict(), "best_seed_42.pt")
                counter = 0
            else:
                counter += 1
        ...
        print('手动终止训练')
        model_save_path = "c_model_interrupted_4.pth"
...
for seed in seed_list:
    ...
    model.load_state_dict(torch.load('model/seed_42.pth'))
    learning_rate = 1e-4
    num_epochs = 2
    ...
    model_save_path = f"model/model_seed_{seed}.pth"  # Change the model name based on the seed
